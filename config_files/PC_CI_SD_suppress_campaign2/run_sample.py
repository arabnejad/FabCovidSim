...
if __name__ == "__main__":
    ...
    #read the random seeds from a file
    seeds_file = os.path.join(args.paramdir, "p_seeds.txt")
    if not os.path.exists(no_int_file):
        print("Unable to find seeds file")
        print("Param directory: {0}".format(args.paramdir))
        print("Looked for: {0}".format(seeds_file))
        exit(1)
        
    fp = open(seeds_file)
    fp.readline() # skip header
    seeds = [int(val) for val in fp.readline().split()]
    fp.close()
    ...
    # network and population binary for non default seeds
    # all new network files are placed in the 'population_newcode' folder

    #binary network file
    network_bin = os.path.join(InputLoadFiles, 
                               "population_newcode/NetworkGB_28T_%d_%d.bin" % (seeds[0], seeds[1],))

    #binary population file
    wpop_bin = os.path.join(InputLoadFiles, 
                            "population_newcode/GB_pop2018_%d_%d.bin" % (seeds[0], seeds[1],))

    #input file for wpop_bin generation
    wpop_txt = os.path.join(InputLoadFiles, 
                            "population_newcode/GB_pop2018_nhs.txt")

    #if we already generated the network file
    if os.path.exists(network_bin): 

        # # network and population binary for the default seeds
        # network_bin = os.path.join(
        ...
        #command to generate the network files:
        #../../covid-sim/build/CovidSimOriginal /c:28 
        #/PP:preGB_R0\=2.0.txt /P:p_NoInt.txt /CLP1:100000 /CLP2:0 /O:NoInt_R0=2.4 
        #/D:GB_pop2018_nhs.txt /M:GB_pop2018.bin /S:NetworkGB_8T.bin 
        #/R:1.2 98798150 729101 17389101 4797132
