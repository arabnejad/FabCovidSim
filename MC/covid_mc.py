...
if init:
    ...
    campaign = uq.Campaign(name='covid' + ID, work_dir=work_dir)
    ...
    sampler = uq.sampling.RandomSampler(vary, max_num=5)  
...
for i in range(n_refinements):
    #increase the max number of samples and run again
    skip = sampler.count
    sampler.max_num += 50
    campaign.draw_samples()
    campaign.populate_runs_dir()
